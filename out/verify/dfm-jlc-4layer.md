...
**FAIL** — 20 PASS, 4 FAIL, 4 CANNOT DETERMINE. Generated 2026-09-04T18:37:59Z.

*Rule set: JLCPCB 4-layer FR4, 1 oz outer / 0.5 oz inner — published capability limits*

Board: 25.6138 x 26.0 mm, 4 copper layers, 0.6 mm, 62 footprints, 50 vias, 257 track segments.

> **80 unconnected item(s)** — the ratsnest. Not a fab-capability failure and not counted above, but this board is not finished.
...
|---|--:|--:|---|---|---|
| **smd_pad_min** — Minimum SMD pad size (each side) | 0.25 | 0.2 | FAIL | geometry | narrowest side of any of 254 SMD pads |
...
| **track_spacing_min** — Minimum track-to-track spacing | 0.09 | — | FAIL | kicad-cli pcb drc | KiCad DRC, constraint `min_clearance` = 0.09 mm: 1 violation(s), e.g. Överträdelse av isolationsavstånd/frigång (nätklass 'Default' frigång 0.0900 mm; faktisk 0.0639 mm) |
| **via_to_track_min** — Via hole to track clearance | 0.2 | — | FAIL | kicad-cli pcb drc | KiCad DRC, constraint `min_hole_clearance` = 0.2 mm: 2 violation(s), e.g. Otillräcklig frigång för hål (mönsterkortsinställningarnas begränsningar för hål frigång 0.2000 mm; faktisk 0.1639 mm) |
| **inner_pth_to_copper_min** — Inner-layer PTH pad hole to copper clearance | 0.3 | — | CANNOT DETERMINE | kicad-cli pcb drc (custom) | KiCad DRC, custom rule `jlc_inner_pth_to_copper` from the .kicad_dru written beside the board: no violation  — AND THIS RULE NEVER FIRED even with its limit multiplied by 40, so either the board has no items it matches, or the condition is one KiCad cannot evaluate. A rule that cannot fail did not pass. |
| **pad_hole_to_hole_min** — PTH pad hole-to-hole spacing | 0.45 | — | CANNOT DETERMINE | kicad-cli pcb drc (custom) | KiCad DRC, custom rule `jlc_pad_hole_to_hole` from the .kicad_dru written beside the board: no violation  — AND THIS RULE NEVER FIRED even with its limit multiplied by 40, so either the board has no items it matches, or the condition is one KiCad cannot evaluate. A rule that cannot fail did not pass. |
| **pth_to_track_min** — PTH to track clearance | 0.28 | — | CANNOT DETERMINE | kicad-cli pcb drc (custom) | KiCad DRC, custom rule `jlc_pth_to_track` from the .kicad_dru written beside the board: no violation  — AND THIS RULE NEVER FIRED even with its limit multiplied by 40, so either the board has no items it matches, or the condition is one KiCad cannot evaluate. A rule that cannot fail did not pass. |
| **smd_pad_to_pad_min** — SMD pad to pad clearance, different nets | 0.15 | — | CANNOT DETERMINE | kicad-cli pcb drc (custom) | KiCad DRC, custom rule `jlc_smd_pad_to_pad` from the .kicad_dru written beside the board: no violation  — AND THIS RULE NEVER FIRED even with its limit multiplied by 40, so either the board has no items it matches, or the condition is one KiCad cannot evaluate. A rule that cannot fail did not pass. |
...
| **track_width_min** — Minimum track width | 0.09 | 0.15 | PASS | geometry (KiCad DRC checks this too) + kicad-cli pcb drc | KiCad DRC, constraint `min_track_width` = 0.09 mm: no violation  ||  geometry: narrowest of 257 track segments |
...
| **via_drill_min** — Minimum via hole (drill) diameter | 0.15 | 0.25 | PASS | geometry | smallest via drill on the board, over 50 vias |
