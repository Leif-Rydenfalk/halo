...
w("## The sounder")
w("")
w(f"**Specified:** {S['specified']}")
w("")
w(f"**Decision:** {S['decision']}")
w("")
...
w("")
w(f"| | 7BB-20-3 (D11a asked for) | {R.get('mpn')} (D20, what is buyable) |")
w("|---|---|---|")
w(f"| Diameter | 20.0 mm | **{R.get('diameter_mm')} mm** |")
w(f"| Total thickness | 0.22 mm | **{R.get('thickness_mm')} mm {R.get('thickness_tol')}** |")
w(f"| Weight | — | {R.get('weight_g')} g |")
w(f"| Ceramic layer | — | {R.get('ceramic_layer_mm')} mm {R.get('ceramic_layer_tol')} |")
w(f"| Brass plate | — | {R.get('brass_mm')} mm ({R.get('brass_note')}) |")
_d = " / ".join(str(x) for x in [R.get("ceramic_diameter_mm"), R.get("electrode_diameter_mm")] if x)
w(f"| Ceramic / electrode Ø | — | {_d} mm |")
fr = (f"**{R['f0_min_hz']:,} / {R['f0_hz']:,} / {R['f0_max_hz']:,} Hz** (min/typ/max)"
      if R.get("f0_min_hz") else f"**{R.get('f0_hz'):,} ± {R.get('f0_tol_hz')} Hz**")
w(f"| Resonant frequency | ~3.6 kHz | {fr} |")
...
w(f"| Operating / storage | — | {R.get('operating_c')} / {R.get('storage_c')} °C |")
...
w(f"**{SA.get('mpn')}** — {SA.get('manufacturer')}, Ø{SA.get('diameter_mm')} mm, "
  ...
  f"{SA.get('capacitance_pf'):,} pF, ≤{SA.get('resonant_impedance_ohm_max')} Ω, "
  + " · ".join(f"{k} {v}" for k, v in (SA.get("stock") or {}).items())
  + f" — [datasheet]({SA.get('datasheet')}) ({SA.get('datasheet_rev')})")
w("")
spx = SA.get("usd_per_unit", {})
w("| | " + R.get("mpn") + " (fitted) | " + SA.get("mpn") + " (second source) |")
w("|---|--:|--:|")
for q in QTYS:
    a_, b_ = px.get(q), spx.get(q)
    d = f" (**−{(a_-b_)/a_*100:.0f}%**)" if a_ and b_ else ""
    w(f"| @{int(q):,} | {money(a_,4)} | {money(b_,4)}{d} |")
w("")
w(SA.get("why", ""))
w("")
w(SA.get("why_not_the_pick", ""))
w("")
w("### Cautions")
w("")
