...
- `fcc-BCGA2187-internal-photo-6.jpg` — **THE MOST USEFUL IMAGE IN THIS SET. Corrected 2026-09-05: the earlier catalogue line saying it had no scale reference was WRONG.** The bare annular main board, both faces of interest unobstructed, **between two steel rulers** — one along the bottom reading cm 1–11 with millimetre ticks and an inch scale, one up the right reading cm 1–7. Two independent scales in one frame, so the pitch cross-checks itself rather than resting on an unverifiable single datum. Apple's own arrows label **Bluetooth Antenna**, **Bluetooth Module**, **UWB Module** and **UWB Antenna** — primary-source identification of antenna positions, from the manufacturer. The centre hole is visibly **a rounded square with a notch at top centre**, NOT a circle: any "centre hole diameter" in this project is the wrong kind of number, and a circle fitted to it would show a small clean residual while being the wrong shape, which is worse than an imprecise number because it looks well-measured. Watermarked "Apple Proprietary and Confidential"; it is nonetheless a public regulatory filing.
